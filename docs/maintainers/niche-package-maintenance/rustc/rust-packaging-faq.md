...
{term}`Metapackages <Metapackage>` carry no files themselves; they only declare dependencies on other packages: in this case, the currently-default versioned `rustc-X.Y` and `cargo-X.Y` binary packages produced by the corresponding versioned `rustc-X.Y` source package.
