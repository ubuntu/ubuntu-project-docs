...
For example, here is the top of :file:`debian/control` from the :pkg:`groff` package, version 1.22.4-10 shipped with Ubuntu 23.04:
...
.. note::
   You may occasionally encounter a package whose ``Maintainer`` field points to neither a Debian nor an ``ubuntu.com`` address. For example, OEM :term:`metapackages <Metapackage>` such as ``oem-qemu-meta`` use ``Commercial Engineering <commercial-engineering@canonical.com>``. These packages follow a streamlined process rather than the usual maintainer conventions. See :ref:`mir-exceptions-oem` for details.
