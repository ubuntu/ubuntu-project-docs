...
If you cannot file a bug using the *Apport* tool, file it via [Launchpad's own bug report form](https://bugs.launchpad.net/ubuntu/+filebug). First determine which package to file against. Read {ref}`how-to-assign-a-bug-to-a-package` for guidance or use [Launchpad's package search feature](https://launchpad.net/ubuntu/). We don't recommend this method for most bug reports because they might be missing crucial information. Use `ubuntu-bug` if you can!
