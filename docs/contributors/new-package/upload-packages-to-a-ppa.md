...
For comprehensive instructions on creating a PPA and uploading packages, see the {external:ref}`Launchpad documentation on uploading to a PPA <upload-a-package-to-a-ppa>`.

## Managing PPAs with `ppa-dev-tools`
...
This produces a `.changes` file in the parent directory. Use `dput` (which provides `dput-ng` on recent Ubuntu releases) to upload it:
...
The `dput` package on recent Ubuntu releases provides `dput-ng`, which can warn you or block uploading of the package if it does not conform to certain standards. To install it separately:
