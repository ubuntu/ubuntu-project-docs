...
* There is no easy front-end which allow users to submit detailed problem reports.
...
If you want to make crash reports of your software even more useful when being reported through Apport, please see {ref}`Apport developer how-to <apport-developer-how-to>`.
...
```{note}
Apport does not trap SIGABRT signals. If you are getting such a signal, then please see {ref}`How to debug an Apport crash <how-to-debug-an-apport-crash>`.
```
...
* `SegvRebazaarL VMA` would mean that a NULL pointer was most likely dereferenced while reading a value.
...
* **Ubuntu Bug Patterns**: [These](https://git.launchpad.net/~ubuntu-bugcontrol/apport/+git/ubuntu-bugpatterns) are patterns for packages (writable by Ubuntu Bug Control) that prevent bugs from being filed by apport.  Complete details are found in the [README](https://git.launchpad.net/~ubuntu-bugcontrol/apport/+git/ubuntu-bugpatterns/tree/README).
...
Note that even if `ulimit` is set to disabled core files (by specifying a core file size of zero using `ulimit -c 0`), Apport will _still_ capture the crash.
...
* `source_totem.py` - asks the reporter questions and gathers different information based on responses in `/usr/share/apport/package-hooks`.

There is also a {ref}`list of packages providing apport hooks <apport-package-hooks>`.

Please see the {ref}`Apport developer how-to <apport-developer-how-to>` for further information.

If a crash or bug report is submitted through Apport, the relevant hooks will be run automatically. If you have an already reported bug that was filed without Apport, and you are interested in the information from those hooks, you can ask the bug reporter to use `apport-collect` _bugnumber_ (see {ref}`Tools <tools>`).
...
* The {ref}`Error tracker <error-tracker>` (Whoopsie) is a newer Ubuntu crash submission system that doesn't require any input from the user and integrates with Apport
* Please do not hesitate to report bugs and feature requests to the [bug tracker](https://launchpad.net/apport/+bugs).
* For retracing Apport-generated bug reports in Launchpad, see {ref}`How to debug an Apport crash <how-to-debug-an-apport-crash>`.
