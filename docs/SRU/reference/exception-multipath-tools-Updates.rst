...
multipath-tools package in Ubuntu LTS releases.
...
From time to time, minor releases will be made on these branches [multipath-tools_readme]_.
...
#. File an MRE bug including the rationale for the upgrade.
   This MRE bug will contain references to previous MREs bugs, as well as a summary of the important bugfixes present in the new microrelease.
   See the SRU template below for more details on how this bug will look like.

#. Once everything is OK, upload the package to the proposed pocket (if it's a non-security upload), and, once approved, keep an eye on the excuses page and investigate any DEP8 failures.
...
As explained above, the testing will be done primarily using a PPA, from which we will run autopkgtests as well as upstream's testsuite during the package build.
