...
Prerequisites and setup are covered in "Requirements" below, model
configuration in "LLM usage by the tool".

<!-- auto-mir-docs:shared-start -->

## Quick start

The commands below assume the requirements are met (see "Requirements") and
run from the tool directory.

Review an existing Launchpad MIR bug:
...
conclusions, and thereby complete it before posting to the Launchpad bug
(see "Output" for what a run produces).
...
Auto-MIR redacts registered credential values from console logs and all
...
The tool is currently in a test phase and only available from this
repository; it is expected to be made available separately after the test
phase. Get it by cloning the repository and changing into the tool directory:

```none
$ git clone https://github.com/ubuntu/ubuntu-project-docs
$ cd ubuntu-project-docs/tools/auto-mir
```
...
- An OpenRouter API key for a full review (see "LLM usage by the tool")
- Enough resources for the default LXD VM: 4 CPUs, 8 GiB memory, and a 20 GiB
  root disk

Install the Python dependencies from the Ubuntu archive:
...
OpenAI-compatible endpoint to evaluate AI-assisted checks and to enrich
drafts. Everything else in a run is deterministic.

A full review uses OpenRouter by default. Read an API key without putting it
in shell history, then export it to the current shell:
...
### The interactive question phase
...
at most the question in flight (see "Interrupted runs and recovery").
...
interrupt a run while it waits, nothing is lost - see "Interrupted runs and
recovery".
...
```
┌─────────────────────────┐  defines  ┌───────────────────────┐
│ auto-mir.py             │◄──────────│ catalog*.yaml files   │
│ orchestrates order and  │           └───────────────────────┘
│ dependencies            │
└────────────┬────────────┘
             ▼
┌──────────────────────────────────────┐  ┌────────────────────────────────┐
│ evidence/                            │  │ orchestration / intake         │
│ abstracts the sources of factual     │  │ reports progress and handles   │
│ data (builds, CVEs, packages, ...).  │  │ bounded operator interaction.  │
└────────────┬─────────────────────────┘  └────────────────────────────────┘
             ▼
┌──────────────────────────────────────┐  ┌────────────────────────────────┐
│ checks/                              │  │ prompts/                       │
│ use data to decide about MIR rules;  │─►│ guide LLM calls and handling   │
│ where interpretation is needed,      │◄─│ of answers.                    │
│ call the LLM.                        │  └────────────────────────────────┘
└────────────┬─────────────────────────┘
             ▼
┌──────────────────────────────────────┐
│ render/                              │
│ converts all insight to full         │
│ report.json and review-draft.txt     │
│ for human finalization.              │
└──────────────────────────────────────┘
```

Architectural references:

- [Check and evidence catalog](CATALOG.md) - the catalog files as the single,
  human-auditable source of check and item definitions.
- [Check evaluation](checks/README.md) - how catalog checks are routed and
  evaluated into findings.
- [Evidence adapters](evidence/README.md) - how factual data is collected
  from host and in-guest sources.
- [Rendering](render/README.md) - how findings become the reviewer draft and
